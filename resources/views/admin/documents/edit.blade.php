...
@section('content')
{{--
    KONSISTEN DENGAN DESAIN CREATE & INDEX:
...
<div class="container-fluid p-4" style="background-color: #f8fafc; min-height: 100vh;">
    ...
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-3 border-bottom border-secondary border-opacity-10">
        <div>
            <h4 class="fw-bold text-dark mb-1">Perbarui Dokumen</h4>
            <p class="text-muted small mb-0">Lakukan perubahan data atau ganti berkas PDF yang sudah ada.</p>
        </div>
            ...
            <div class="col-lg-8">
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
                    <div class="card-header bg-white py-3 px-4 border-bottom">
                        <h6 class="mb-0 fw-bold text-dark"><i class="bi bi-pencil-square me-2 text-primary"></i>Formulir Perubahan Data</h6>
                    </div>
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="title" class="form-control form-control-lg border-0 bg-light fw-bold text-dark shadow-none @error('title') is-invalid @enderror"
                                   placeholder="Masukkan judul dokumen..." value="{{ old('title', $document->title) }}" required style="font-size: 1.1rem;">
                        ...
                        <div class="mb-0">
                            ...
                            <textarea name="description" rows="5" class="form-control border-0 bg-light shadow-none"
                                      placeholder="Tambahkan keterangan jika perlu...">{{ old('description', $document->description) }}</textarea>
                        </div>
                    </div>
                </div>
            </div>

            {{-- KOLOM KANAN: UPDATE BERKAS --}}
            <div class="col-lg-4">
                <div class="card border-0 shadow-sm rounded-4 bg-white overflow-hidden mb-4">
                    <div class="card-header bg-white py-3 px-4 border-bottom text-primary fw-bold">
                        <i class="bi bi-arrow-repeat me-2"></i>Update Berkas
                    </div>
                    <div class="card-body p-4">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="category" class="form-select border-0 bg-light shadow-none py-2 rounded-3 fw-medium @error('category') is-invalid @enderror" required>
                                <option value="berkala" {{ $document->category == 'berkala' ? 'selected' : '' }}>Informasi Berkala</option>
                                <option value="serta-merta" {{ $document->category == 'serta-merta' ? 'selected' : '' }}>Informasi Serta Merta</option>
                                <option value="setiap-saat" {{ $document->category == 'setiap-saat' ? 'selected' : '' }}>Informasi Setiap Saat</option>
                                <option value="dikecualikan" {{ $document->category == 'dikecualikan' ? 'selected' : '' }}>Informasi Dikecualikan</option>
                                <option value="sop-ppid" {{ $document->category == 'sop-ppid' ? 'selected' : '' }}>SOP Layanan</option>
                            </select>
                        ...
                        <div class="mt-4 text-center">
                            <small class="text-muted" style="font-size: 10px;">
                                Terakhir diperbarui: <br><strong>{{ $document->updated_at->format('d M Y, H:i') }} WITA</strong>
                            </small>
                        </div>
                    </div>
                </div>
